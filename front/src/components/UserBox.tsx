...
const UserBox: React.FC<Props> = ({ user, host }: Props) => {
    return (
        <div className="shadow shadow-black bg-white w-32 lg:w-56 xl:w-64 border-2 border-black p-3 flex flex-col items-center h-full">
            <p className="text-xs lg:text-xl tracking-wide text-center"><b>{user.name}{host === user.name && ' (Host)'}</b></p>
            <img 
                src={user.image} 
                alt={`${user.name} player icon`} 
                className="max-w-12 lg:max-w-44 m-3 border-2 border-black shadow shadow-black"
            />
