...
const Landing: React.FC = () => {
    ...
    const handleJoinRoom = async (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const isRoomCodeValid = (await axios.get(`http://localhost:5000/isRoomCodeValid?roomCode=${roomCode}`)).data;
        if(isRoomCodeValid) {
    ...
    const handleCreateRoom = async (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const newCode = (await axios.get('http://localhost:5000/roomcode')).data;
